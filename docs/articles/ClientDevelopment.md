...
- [Tutorials](/tutorials/SampleCompany)
...
For UA Server discovering you can use the DiscoverServersAsync() method. To be able to find an UA server, all UA Servers running on a machine should register with the UA Local Discovery Server using the Stack API.
...
```
    ...
    IList<string> localServers = await UaClientUtils.DiscoverServersAsync(config, telemetry).ConfigureAwait(false);
