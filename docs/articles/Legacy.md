...
The support for the features described in this chapter will only be guaranteed up to version 4.3. From version 5.0 on they will no longer be supported. 
...
The following NuGet package will be no longer available:

| **Name**                                       | **Description**                                                                                    |
|:-----------------------------------------------|:--------------------------------------------------------------------------------------------------:|
| **Technosoftware.UaSolutions.UaBaseServer**     | The OPC UA Base Server Class library used by the samples explained in [Server Design based on UaBaseServer](#server-design-based-on-uabaseserver).                    |

### Directory Structure

We provide an online help for the current version: [OPC UA Solution NET Online Help](https://technosoftware.com/help/UaSolutions/42/) which also contains updated information about the directory structure.
...
## Server Design based on UaBaseServer
...
| *.Config.xml               | Contains the configuration of the OPC UA Server, e.g. ApplicationName. For the sample server it is named: SampleCompany.SampleServer.Config.xml                                                                                                                                                      |
| *Server.cs                 | Implements the basic OPC UA Server and overrides the UaBaseServer class. For the sample server it is named: SampleCompany.SampleServer.cs                                                                                                                                                            |
| *ServerNodeManager.cs      | Implements the OPC UA NodeManager and overrides the UaBaseNodeManager class. For the sample server it is named: SampleCompany.SampleServerNodeManager.cs                                                                                                                                             |
...
2. Rename the SampleServer class to your needs for the whole project.
3. Open the SampleCompany.SampleServerNodeManager.cs file with Visual Studio 2022.
4. Rename the SampleServerNodeManager class to your needs for the whole project.
5. Open the Program.cs file with Visual Studio 2022.
6. Rename the Task SampleServer method for the whole project.
