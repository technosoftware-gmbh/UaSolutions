...
namespace Technosoftware.UaClient.Tests
{
    ...
    public class SubscriptionUnitTests
    {
        ...
        private static IUaSession BuildSessionMock(Func<uint, uint, bool> republishHandler = null, Action<Mock<IUaSession>> setup = null)
        {
            uint subscriptionIdSeed = 0u;

            var session = new Mock<IUaSession>();
            if (republishHandler is not null)
            {
                session
                    .Setup(x => x.RepublishAsync(It.IsAny<uint>(), It.IsAny<uint>(), It.IsAny<CancellationToken>()))
                    .ReturnsAsync<uint, uint, CancellationToken, IUaSession, (bool, ServiceResult)>((
                        subscriptionId,
                        sequenceNumber,
                        ct) =>
                    {
        ...
        public async Task RespectsStateOfSessionDuringKeepAliveCalls(KeepAliveTestDataProvider testData, CancellationToken ct)
        {
            ...
            IUaSession session = BuildSessionMock(
                setup: mock =>
                {
