...
namespace Technosoftware.UaClient.Tests
{
    ...
    public class ClientTest : ClientTestFramework
    {
        ...
        public async Task InvalidConfiguration()
        {
            ...
            ApplicationConfiguration config = await applicationInstance.CreateApplicationConfigurationManager(ClientFixture.Config.ApplicationUri, ClientFixture.Config.ProductUri)
                .AsClient()
        ...
        public async Task ConnectAndClose(string securityPolicy)
        {
            ...
            Session.SessionClosingEvent += OnSessionClosing;
        ...
        public async Task ConnectAndReconnectAsync(bool reconnectAbort, bool useMaxReconnectPeriod)
        {
            ...
            session.SessionConfigurationChangedEvent += (object sender, EventArgs e) => { sessionConfigChanged++; };

            int sessionClosing = 0;
            session.SessionClosingEvent += (object sender, EventArgs e) => { sessionClosing++; };
        ...
        public async Task ReconnectSessionOnAlternateChannel(bool closeChannel, bool asyncReconnect)
        {
            ...
            // the active channel
            IUaSession session1 = await ClientFixture.ConnectAsync(ServerUrl, SecurityPolicies.Basic256Sha256, Endpoints).ConfigureAwait(false);
        ...
        public async Task ReconnectSessionOnAlternateChannelWithSavedSessionSecrets(string securityPolicy, bool anonymous, bool asyncReconnect)
        {
            ...
            // the active channel
            IUaSession session1 = await ClientFixture.ConnectAsync(endpoint, userIdentity).ConfigureAwait(false);
            Assert.NotNull(session1);
            ...
            IUaSession session2 = ClientFixture.CreateSession(channel2, sessionConfiguration.ConfiguredEndpoint);
        ...
        public async Task RecreateSessionWithRenewUserIdentity(bool async)
        {
            ...
            // the active channel
            IUaSession session1 = await ClientFixture.ConnectAsync(endpoint, userIdentityAnonymous).ConfigureAwait(false);
            Assert.NotNull(session1);
            ...
            UaClient.Session session2;
            if (async)
            {
                session2 = await UaClient.Session.RecreateAsync((Session)((TraceableSession)session1).Session).ConfigureAwait(false);
            }
            else
            {
                session2 = UaClient.Session.Recreate((Session)((TraceableSession)session1).Session);
            }
            ...
            UaClient.Session session3;
            if (async)
            {
                session3 = await UaClient.Session.RecreateAsync((Session)((TraceableSession)session1).Session, channel2).ConfigureAwait(false);

            }
            else
            {
                session3 = UaClient.Session.Recreate((Session)((TraceableSession)session1).Session, channel2);
            }
        ...
        public async Task LoadStandardDataTypeSystem()
        {
            ...
            Assert.AreEqual((StatusCode)StatusCodes.BadNodeIdInvalid, (StatusCode)sre.StatusCode);
            var typeSystem = await Session.LoadDataTypeSystemAsync().ConfigureAwait(false);
            Assert.NotNull(typeSystem);
            typeSystem = await Session.LoadDataTypeSystemAsync(ObjectIds.OPCBinarySchema_TypeSystem).ConfigureAwait(false);
            Assert.NotNull(typeSystem);
            typeSystem = await Session.LoadDataTypeSystemAsync(ObjectIds.XmlSchema_TypeSystem).ConfigureAwait(false);
            Assert.NotNull(typeSystem);
        }
        ...
        private Subscription MockOutSubscriptionSession(Moq.Mock<IUaSession> sessionMock)
        {
