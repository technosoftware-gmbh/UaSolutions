...
namespace Technosoftware.ClientGateway.Hda
{
    ...
    /// </summary>
    /// <exclude />
    internal class HdaSubscribeAttributeRequest
    {
        #region Constructors
        /// <summary>
        ...
        public HdaSubscribeAttributeRequest(string itemId, ITelemetryContext telemetry)
        {
            m_itemId = itemId;
            m_telemetry = telemetry;
            m_logger = m_telemetry.CreateLogger<HdaSubscribeAttributeRequest>();
        }
        ...
        public void QueueValues(UaServerContext context, HdaReadRequest request, bool initialUpdate)
        {
            for (int ii = 0; ii < m_monitoredItems.Count; ii++)
            {
        ...
        /// <param name="request">The request.</param>
        /// <param name="monitoredItem">The monitored item.</param>
        private void QueueValue(UaServerContext context, HdaReadRequest request, Technosoftware.UaServer.UaMonitoredItem monitoredItem)
        {
            UaNodeHandle handle = monitoredItem.ManagerHandle as UaNodeHandle;
    ...
    internal class HdaSubscribeRequestManager : IDisposable
    {
        ...
        /// <param name="telemetry"></param>
        public HdaSubscribeRequestManager(UaServerContext context, int localeId, ComHdaClientConfiguration configuration, ITelemetryContext telemetry)
        {
            m_context = context;
            m_localeId = localeId;
            ...
            m_telemetry = telemetry;
            m_logger = m_telemetry.CreateLogger<HdaSubscribeRequestManager>();
        }
        #endregion Constructors
        ...
        private HdaSubscribeAttributeRequest FindAttributeRequest(string itemId, bool create)
        {
            lock (m_lock)
            {
                ...
                if (!m_requests.TryGetValue(itemId, out requests))
                {
                    ...
                    requests = new HdaSubscribeAttributeRequest(itemId, m_telemetry);
        ...
        /// <param name="monitoredItem">The monitored item.</param>
        private HdaSubscribeAttributeRequest Add(NodeState source, Technosoftware.UaServer.UaMonitoredItem monitoredItem)
        {
            lock (m_lock)
            {
        ...
        /// <param name="source">The source.</param>
        /// <param name="monitoredItem">The monitored item.</param>
        private HdaSubscribeAttributeRequest Modify(NodeState source, Technosoftware.UaServer.UaMonitoredItem monitoredItem)
        {
            lock (m_lock)
            {
                // get the HDA item id.
        ...
        /// <param name="source">The source.</param>
        /// <param name="monitoredItem">The monitored item.</param>
        private HdaSubscribeAttributeRequest Remove(NodeState source, Technosoftware.UaServer.UaMonitoredItem monitoredItem)
        {
            lock (m_lock)
            {
        ...
        private void ApplyChanges(Dictionary<string, HdaSubscribeAttributeRequest> requests, bool initialUpdateRequired)
        {
            ...
            lock (m_lock)
            {
                ...
                if (m_attributeScanTimer == null)
                {
                    ...
                    {
                        m_attributeScanTimer = new Timer(
                            OnScanAttributes,
                            null,
                            m_configuration.AttributeSamplingInterval,
                            m_configuration.AttributeSamplingInterval);
                    }
                }
                else
        ...
        private void DoScan(Dictionary<string, HdaSubscribeAttributeRequest> requests, bool initialUpdate)
        {
            try
            {
                ...
                lock (m_lock)
                {
                    ...
                    foreach (KeyValuePair<string, HdaSubscribeAttributeRequest> entry in requests)
                    {
                        ...
                        {
                            Technosoftware.UaServer.UaMonitoredItem monitoredItem = entry.Value.MonitoredItems[ii];
            ...
            {
                m_logger.LogError(e, "Unexpected error scanning attributes in HDA COM server.");
            }
        }
