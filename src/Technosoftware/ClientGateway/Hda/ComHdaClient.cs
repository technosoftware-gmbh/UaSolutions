...
namespace Technosoftware.ClientGateway.Hda
{
    ...
    /// </summary>
    /// <exclude />
    internal class ComHdaClient : ComClient
    {
        #region Constructors
        /// <summary>
        ...
        public ComHdaClient(ComHdaClientConfiguration configuration, ITelemetryContext telemetry) : base(configuration, telemetry)
        {
        ...
        #endregion ServerStatus Structure
        ...
        internal static StatusCode MapErrorCodeToUpdateStatus(int error)
        {
            ...
            if (error < 0)
            {
                switch (error)
                {
                    ...
                    case ResultIds.E_RANGE:
                    { return StatusCodes.BadOutOfRange; }
                    case ResultIds.E_BADTYPE:
                    { return StatusCodes.BadTypeMismatch; }
                    case ResultIds.DISP_E_OVERFLOW:
                    { return StatusCodes.BadOutOfRange; }
                    case ResultIds.DISP_E_TYPEMISMATCH:
                    { return StatusCodes.BadTypeMismatch; }
                    case ResultIds.E_DATAEXISTS:
                    { return StatusCodes.BadEntryExists; }
                    case ResultIds.E_NODATAEXISTS:
            ...
            out int[] errors)
        {
            ...
            for (int ii = 0; ii < dwNumItems; ii++)
            {
                ...
                vDataValues[ii] = ComUtils.GetVARIANT(value.WrappedValue, Telemetry);
        ...
        private StatusCode ReadProcessed(HdaHistoryReadProcessedRequest request)
        {
            ...
            if (errors[0] < 0)
            {
                if (errors[0] == ResultIds.E_NOT_AVAIL || errors[0] == ResultIds.E_NOTIMPL || errors[0] == ResultIds.E_NOTSUPPORTED)
                {
