...
namespace Technosoftware.ClientGateway.Da
{
    ...
    internal class ComDaGroup : ComObject
    {
        ...
        public ComDaGroup(ComDaClient server, bool callbacksRequired, ITelemetryContext telemetry) : base(telemetry)
        {
            m_server = server;
            ...
            m_logger = telemetry.CreateLogger<ComDaGroup>();
        ...
        public void SetMonitoredItems(GroupItem item, Technosoftware.UaServer.UaMonitoredItem[] monitoredItems)
        {
        ...
        {
            public Technosoftware.UaServer.UaMonitoredItem[] MonitoredItems;
        ...
        internal void OnDataChange(int[] clientHandles, DaValue[] values)
        {
            ...
            lock (m_monitoredItems)
            {
                for (int ii = 0; ii < clientHandles.Length; ii++)
                {
                    ...
                    Technosoftware.UaServer.UaMonitoredItem[] monitoredItems = info.MonitoredItems;
        ...
        public void Create()
        {
            ...
            if (m_monitoredItems != null)
            {
                try
                {
                    m_callback = new ComDaDataCallback(this, m_telemetry);
                }
                catch (Exception e)
                {
                    m_logger.LogError("Could not establish IOPCDataCallback.", e);
                }
            }
        }
        ...
        public void AddItems()
        {
            ...
            Technosoftware.Rcw.OPCITEMDEF[] definitions = new Technosoftware.Rcw.OPCITEMDEF[count];

            for (int ii = 0; ii < count; ii++)
            {
                definitions[ii] = new Technosoftware.Rcw.OPCITEMDEF();
        ...
        internal static int[] GetItemResults(ref IntPtr pInput, int count, bool deallocate)
        {
            ...
            if (pInput != IntPtr.Zero && count > 0)
            {
                ...
                for (int ii = 0; ii < count; ii++)
                {
                    Technosoftware.Rcw.OPCITEMRESULT result = (Technosoftware.Rcw.OPCITEMRESULT)Marshal.PtrToStructure(
                        pos,
                        typeof(Technosoftware.Rcw.OPCITEMRESULT));
                    ...
                    if (deallocate)
                    {
                        ...
                        Marshal.DestroyStructure(pos, typeof(Technosoftware.Rcw.OPCITEMRESULT));
                    }

                    if (ii < count - 1)
                    {
                        pos = (IntPtr)(pos.ToInt32() + Marshal.SizeOf(typeof(Technosoftware.Rcw.OPCITEMRESULT)));
                    }
                }
        ...
        internal static DaValue[] GetItemValues(ref IntPtr pInput, int count, bool deallocate)
        {
            ...
            if (pInput != IntPtr.Zero && count > 0)
            {
                ...
                for (int ii = 0; ii < count; ii++)
                {
                    Technosoftware.Rcw.OPCITEMSTATE result = (Technosoftware.Rcw.OPCITEMSTATE)Marshal.PtrToStructure(
                        pos,
                        typeof(Technosoftware.Rcw.OPCITEMSTATE));
                    ...
                    if (deallocate)
                    {
                        Marshal.DestroyStructure(pos, typeof(Technosoftware.Rcw.OPCITEMSTATE));
                    }

                    pos = (IntPtr)(pos.ToInt32() + Marshal.SizeOf(typeof(Technosoftware.Rcw.OPCITEMSTATE)));
                }
