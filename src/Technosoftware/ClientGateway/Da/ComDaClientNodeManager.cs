...
namespace Technosoftware.ClientGateway.Da
{
    ...
    /// </summary>
    /// <exclude />
    internal class ComDaClientNodeManager : ComClientNodeManager
    {
        #region Constructors
        /// <summary>
        /// Initializes the node manager.
        /// </summary>
        public ComDaClientNodeManager(IUaServerData server, string namespaceUri, ComDaClientConfiguration configuration, bool ownsTypeModel)
        :
            base(server, namespaceUri, ownsTypeModel)
        {
            SystemContext.SystemHandle = m_system = new ComDaClientManager(server.Telemetry);
            ...
            m_subscriptionManagers[String.Empty] = new SubscribeRequestManager(SystemContext, null, 1000, ServerData.Telemetry);
        ...
        public void OnServerReconnected(object state)
        {
            try
            {
                foreach (SubscribeRequestManager manager in m_subscriptionManagers.Values)
                {
                    ...
                    {
                        m_logger.LogError(e, "Could not re-create subscription after reconnect for locale {0}.", new CultureInfo(manager.LocaleId).DisplayName);
                    }
                }
            }
            catch (Exception e)
            {
                m_logger.LogError(e, "Could not re-create subscription after reconnect.");
            }
        }
        ...
        protected override UaNodeHandle GetManagerHandle(UaServerContext context, NodeId nodeId, IDictionary<NodeId, NodeState> cache)
        {
            lock (Lock)
            {
                ...
                if (PredefinedNodes != null)
                {
                    ...
                    {
                        UaNodeHandle handle = new UaNodeHandle();
        ...
        /// </summary>
        protected virtual void AddAdditionalElementReferences(UaServerContext context, NodeState node)
        {
            // TBD
        }
            ...
            IDictionary<NodeId, NodeState> cache)
        {
            ...
            ComDaClient client = system.SelectClient((UaServerContext)SystemContext, false);
        ...
        protected override void OnCreateMonitoredItemsComplete(UaServerContext context, IList<IUaMonitoredItem> monitoredItems)
        {
            ...
            {
                m_subscriptionManagers[client.Key] = manager = new SubscribeRequestManager(context, client, 1000, ServerData.Telemetry);
            }
        ...
        protected override void OnModifyMonitoredItemsComplete(UaServerContext context, IList<IUaMonitoredItem> monitoredItems)
        {
            ...
            Dictionary<string, List<IUaMonitoredItem>> monitoredItemsByLocaleId = new Dictionary<string, List<IUaMonitoredItem>>();
            ...
            for (int ii = 0; ii < monitoredItems.Count; ii++)
            {
                ...
                {
                    monitoredItemsByLocaleId[manager.Key] = subset = new List<IUaMonitoredItem>();
                }
            ...
            // modify the the item.
            foreach (KeyValuePair<string, List<IUaMonitoredItem>> entry in monitoredItemsByLocaleId)
            {
                SubscribeRequestManager manager = null;
        ...
        public static int LocalToRemoteValue(Variant srcValue, out object dstValue)
        {
            ...
            Opc.Ua.TypeInfo srcType = srcValue.TypeInfo;

            if (srcType == null)
            {
                srcType = Opc.Ua.TypeInfo.Construct(srcValue.Value);
            }
            ...
            try
            {
                ...
                {
                    dstValue = Opc.Ua.TypeInfo.CastArray((Array)srcValue.Value, BuiltInType.Variant, BuiltInType.Null, LocalToRemoteValue);
                    return ResultIds.S_OK;
                }
            }
            catch (Exception)
        ...
        private static object LocalToRemoteValue(object srcValue, BuiltInType srcType, BuiltInType dstType)
        {
            ...
            {
                Opc.Ua.TypeInfo typeInfo = Opc.Ua.TypeInfo.Construct(srcValue);
                srcType = typeInfo.BuiltInType;

                if (typeInfo.ValueRank != ValueRanks.Scalar)
                {
                    return Opc.Ua.TypeInfo.CastArray((Array)srcValue, srcType, BuiltInType.Null, LocalToRemoteValue);
                }
        ...
        private static object RemoteToLocalValue(object srcValue, BuiltInType srcType, BuiltInType dstType)
        {
            ...
            if (typeof(decimal[]).IsInstanceOfType(srcValue))
            {
                return Opc.Ua.TypeInfo.CastArray((Array)srcValue, BuiltInType.Null, BuiltInType.String, RemoteToLocalValue);
            }
