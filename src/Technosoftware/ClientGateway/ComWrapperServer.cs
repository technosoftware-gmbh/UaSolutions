...
namespace Technosoftware.ClientGateway
{
    ...
    /// </para>
    /// </remarks>
    public partial class ComWrapperServer : UaStandardServer
    {
        #region Public Interface
        /// <summary>
        /// 
        /// </summary>
        public ComWrapperServerConfiguration WrapperConfiguration { get; set; }
            ...
            ApplicationConfiguration configuration)
        {
            m_logger.LogInformation(
                ...
                "Creating the ComWrapper Server Node Manager.");

            List<IUaNodeManager> nodeManagers = new List<IUaNodeManager>();
            m_availableLocales = new List<string>();

            // get the configuration for the wrapper.
            ComWrapperServerConfiguration wrapperConfiguration = WrapperConfiguration;

            if (wrapperConfiguration == null)
            {
                wrapperConfiguration = configuration.ParseExtension<ComWrapperServerConfiguration>();
            }
                ...
                foreach (ComClientConfiguration clientConfiguration in wrapperConfiguration.WrappedServers)
                {
                        ...
                        foreach (string locale in clientConfiguration.AvailableLocales)
                        {
                            ...
                            catch (Exception e)
                            {
                                m_logger.LogError(
                                    ...
                                    e,
                                    "Can't process an invalid locale: {0}.", locale);
                            }
                        }
                    }
                    ...
                    if (clientConfiguration is ComDaClientConfiguration)
                    {
                        ...
                        if (namespaceUris.ContainsKey(namespaceUri))
                        {
                            m_logger.LogInformation(
                                Utils.TraceMasks.StartStop,
                                "COM server has already been wrapped {0}.", namespaceUri);
                            continue;
                        }
        ...
        protected override ServerProperties LoadServerProperties()
        {
            ...
            properties.ManufacturerName = "Technosoftware GmbH";
            properties.ProductName = "Technosoftware OPC OPC UA Server Gateway .NET";
            properties.ProductUri = "http://technosoftware.com/Gateway/ClassicDServer/v1.0";
        ...
        /// </summary>
        protected override void OnNodeManagerStarted(IUaServerData server)
        {
            ...
            {
                lock (CurrentInstance.DiagnosticsNodeManager.Lock)
                {
                    // get the LocaleIdArray property.
                    BaseVariableState variable = CurrentInstance.DiagnosticsNodeManager
            ...
            m_userNameValidator = new UserNameValidator(Configuration.ApplicationName, server.Telemetry);
        ...
        {
            // verify session
            this.CurrentInstance.SessionManager.ImpersonateUser += OnImpersonateUser;
        }
        ...
        void OnImpersonateUser(object sender, ImpersonateUserEventArgs args)
        {
                    ...
                    {   // Bad user access denied.
                        ...
                        throw new ServiceResultException(
                            new ServiceResult(
                                ...
                                new StatusCode(StatusCodes.BadUserAccessDenied, "InvalidUserInformation"),
