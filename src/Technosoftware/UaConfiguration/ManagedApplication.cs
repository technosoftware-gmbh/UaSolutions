...
namespace Technosoftware.UaConfiguration
{
    ...
    public class ManagedApplication 
    {
        ...
        /// </summary>
        public ManagedApplication(ITelemetryContext telemetry)
        {
            m_telemetry = telemetry;
            m_logger = telemetry.CreateLogger<ManagedApplication>();
        }
        ...
        public void Save(string filePath)
        {
            if (String.IsNullOrEmpty(filePath))
            {
                ...
                {
                    filePath = Utils.GetAbsoluteDirectoryPath("%CommonApplicationData%\\Technosoftware\\Applications\\", false, false, true);
        ...
        public void SetExecutableFile(string filePath)
        {
            ...
            m_logger.LogInformation(1, "APPCONFIG={ConfigFile}", configFile);
            ...
            if (configFile.Exists)
            {
                ...
                m_logger.LogInformation(1, "UACONFIG={ConfigurationPath}", configurationPath);
        ...
        private Opc.Ua.Security.SecuredApplication GetApplicationSettings(string filePath)
        {
            ...
            return new Opc.Ua.Security.SecurityConfigurationManager(m_telemetry).ReadConfiguration(absolutePath);
        }
        ...
        private string GetConfigFileFromAppConfig(FileInfo appConfigFile)
        {
            try
            {
                var src = appConfigFile.Open(FileMode.Open, FileAccess.Read, FileShare.Read);
                TextReader tr = new StreamReader(src);
                XmlReader reader = XmlReader.Create(tr, new XmlReaderSettings() { XmlResolver = null });
                XmlSchema schema = XmlSchema.Read(reader, null);

                // XmlTextReader reader = new XmlTextReader(appConfigFile.Open(FileMode.Open, FileAccess.Read, FileShare.Read));
