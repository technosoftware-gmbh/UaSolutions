...
namespace Technosoftware.UaServer
{
    ...
    public class Subscription : IUaSubscription
    {
            ...
            uint maxMessageCount)
        {
            ...
            m_itemsToPublish = new LinkedList<IUaMonitoredItem>();
            m_itemsToTrigger = [];

            // m_itemsReadyToPublish         = new Queue<IUaMonitoredItem>();
            // m_itemsNotificationsAvailable = new LinkedList<IUaMonitoredItem>();
            m_sequenceNumber = 1;
        ...
        /// </summary>
        public Subscription(IUaServerData server, IUaStoredSubscription storedSubscription)
        {
            if (server.IsRunning)
            {
        ...
        public UaPublishingState PublishTimerExpired()
        {
            lock (m_lock)
            {
                ...
                if (m_publishingEnabled && Session != null)
                {
                    ...
                    while (current != null)
                    {
                        LinkedListNode<IUaMonitoredItem> next = current.Next;
            ...
            out DiagnosticInfoCollection diagnosticInfos)
        {
            ...
            lock (m_lock)
            {
                ...
                for (int ii = 0; ii < count; ii++)
                {
                    ...
                    IUaMonitoredItem monitoredItem = node.Value;
            ...
            out DiagnosticInfoCollection diagnosticInfos)
        {
            ...
            lock (m_lock)
            {
                ...
                for (int ii = 0; ii < count; ii++)
                {
                    ...
                    foreach (KeyValuePair<uint, List<IUaTriggeredMonitoredItem>> item in m_itemsToTrigger)
                    {
        ...
        /// </summary>
        public IUaStoredSubscription ToStorableSubscription()
        {
            var monitoredItemsToStore = new List<IUaStoredMonitoredItem>();

            foreach (KeyValuePair<uint, LinkedListNode<IUaMonitoredItem>> kvp in m_monitoredItems)
            {
            ...
            IEnumerable<IUaStoredMonitoredItem> storedMonitoredItems)
        {
            ...
            lock (m_lock)
            {
                foreach (IUaMonitoredItem monitoredItem in monitoredItems)
                {
                    // skip UaMonitoredItem if recreation failed
                    if (monitoredItem == null)
                    {
                        continue;
                    }
