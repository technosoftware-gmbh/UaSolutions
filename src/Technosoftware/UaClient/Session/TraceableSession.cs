...
namespace Technosoftware.UaClient
{
    ...
    public class TraceableSession : IUaSession
    {
        ...
        /// <inheritdoc/>
        public IUaSession Session => m_session;

        #region IUaSession interface
        /// <inheritdoc/>
        public event EventHandler<SessionKeepAliveEventArgs> SessionKeepAliveEvent
        {
            add => m_session.SessionKeepAliveEvent += value;
            remove => m_session.SessionKeepAliveEvent -= value;
        }

        /// <inheritdoc/>
        public event EventHandler<SessionNotificationEventArgs> SessionNotificationEvent
        {
            add => m_session.SessionNotificationEvent += value;
            remove => m_session.SessionNotificationEvent -= value;
        }

        /// <inheritdoc/>
        public event EventHandler<SessionPublishErrorEventArgs> SessionPublishErrorEvent
        {
            add => m_session.SessionPublishErrorEvent += value;
            remove => m_session.SessionPublishErrorEvent -= value;
        }

        /// <inheritdoc/>
        public event EventHandler<PublishSequenceNumbersToAcknowledgeEventArgs> PublishSequenceNumbersToAcknowledgeEvent
        {
            add => m_session.PublishSequenceNumbersToAcknowledgeEvent += value;
            remove => m_session.PublishSequenceNumbersToAcknowledgeEvent -= value;
        }

        /// <inheritdoc/>
        public event EventHandler SubscriptionsChangedEvent
        {
            add => m_session.SubscriptionsChangedEvent += value;
            remove => m_session.SubscriptionsChangedEvent -= value;
        }

        /// <inheritdoc/>
        public event EventHandler SessionClosingEvent
        {
            add => m_session.SessionClosingEvent += value;
            remove => m_session.SessionClosingEvent -= value;
        }

        /// <inheritdoc/>
        public event EventHandler SessionConfigurationChangedEvent
        {
            add => m_session.SessionConfigurationChangedEvent += value;
            remove => m_session.SessionConfigurationChangedEvent -= value;
        }

        /// <inheritdoc/>
        public event RenewUserIdentity RenewUserIdentityEvent
        {
            add => m_session.RenewUserIdentityEvent += value;
            remove => m_session.RenewUserIdentityEvent -= value;
        }
        ...
        /// <inheritdoc/>
        public IUaNodeCache NodeCache => m_session.NodeCache;
        ...
        public async Task<DataDictionary> FindDataDictionaryAsync(NodeId descriptionId, CancellationToken ct = default)
        {
            using (Activity activity = ActivitySource.StartActivity())
            {
                return await m_session.FindDataDictionaryAsync(descriptionId, ct).ConfigureAwait(false);
            }
        }
        ...
        public async Task<Dictionary<NodeId, DataDictionary>> LoadDataTypeSystemAsync(NodeId dataTypeSystem = null, CancellationToken ct = default)
        {
            using (Activity activity = ActivitySource.StartActivity())
            {
                return await m_session.LoadDataTypeSystemAsync(dataTypeSystem, ct).ConfigureAwait(false);
            }
        }
                                ...
                                object userData)
        {
            using (Activity activity = ActivitySource.StartActivity())
            {
                return m_session.BeginReadValues(variableIds, maxAge, timestampsToReturn, callback, userData);
            }
        }
            ...
            IAsyncResult result)
        {
            using (Activity activity = ActivitySource.StartActivity())
            {
                return m_session.EndReadValues(result);
            }
        }
        ...
        public StatusCode WriteValue(NodeId nodeId, DataValue value)
        {
            using (Activity activity = ActivitySource.StartActivity())
            {
                return m_session.WriteValue(nodeId, value);
            }
        }
        ...
        public List<StatusCode> WriteValues(IList<NodeId> nodeIds, IList<DataValue> dataValues)
        {
            using (Activity activity = ActivitySource.StartActivity())
            {
                return m_session.WriteValues(nodeIds, dataValues);
            }
        }
                                ...
                                object userData)
        {
            using (Activity activity = ActivitySource.StartActivity())
            {
                return m_session.BeginWriteValues(nodeIds, dataValues, callback, userData);
            }
            ...
            IAsyncResult result)
        {
            using (Activity activity = ActivitySource.StartActivity())
            {
                return m_session.EndWriteValues(result);
            }
        }
        #endregion
    }
